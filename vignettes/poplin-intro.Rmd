---
title: "Processing quantitative metabolomics data with the poplin package"
author:
  - name: Jaehyun Joo
...
The `poplin` package provides basic tools for imputation, normalization, and
...
The package can be installed with `r CRANpkg("BiocManager")` by 
issuing `BiocManager::install("poplin")`.

# Data preparation

To demonstrate the use of the `poplin` functions, we will use the 
...
Each point (label) represents a sample and data ellipses can be visualized. 
...
features group whereas `cor_grouping = louvain` further applies the Louvain 
...
function. A group of features in the same feature group will be plotted with
