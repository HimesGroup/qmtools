...
##'   to the imputation when Euclidean distance is used. The imputed values
##'   are re-transformed so that they are on the original scales.
...
.impute_knn_euclidean <- function(x, k, by = c("feature", "sample"),
  ...
  if (by == "feature") {
    if (scale) {
      ...
      .scale_recover(res, attr(xs, "scaled:center"), attr(xs, "scaled:scale"))
    } else{
    ...
    if (scale) {
      xs <- scale(t(x))
      res <- MsCoreUtils::impute_knn(xs, k = k, ...)
      t(.scale_recover(res, attr(xs, "scaled:center"), attr(xs, "scaled:scale")))
    } else{
